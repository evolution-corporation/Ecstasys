...
export const setChangedAccountData = createAction(
  AccountAction.setChangedData,
  (changedData: ChangedAccountData) => ({
...
export const removeChangedAccountData = createAction(
  AccountAction.removeChangedData
);

export const saveChangeAccountData = createAsyncThunk<
  AccountJSON,
  undefined,
  AsyncThunkConfig
>(AccountAction.saveChangeData, async (_, { getState }) => {
  const { changedAccountData } = getState().account;
  return await API.update(
    changedAccountData.nickname,
    changedAccountData.birthday
      ? new Date(changedAccountData.birthday)
      : undefined,
...
>(
  ...
  async ({ birthday, nickname, image }) =>
    await API.registration(nickname, birthday, image)
);
...
export const initializationAccount = createAsyncThunk<
  [AccountJSON | null, AccountStatus],
  undefined,
  AsyncThunkConfig
>(AccountAction.initialization, async () => {
  const user = auth().currentUser;
  if (user === null) {
    return [null, AccountStatus.NO_AUTHENTICATION];
  } else {
    const user = await API.authentication();
    return [
      user,
      user ? AccountStatus.REGISTRATION : AccountStatus.NO_REGISTRATION,
    ];
  }
});
