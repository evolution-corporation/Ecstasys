...
export function ConverterUserDataToApplication(data: UserDataServer, imageType: 'base64' | 'URL' = 'URL'): UserDataApplication {
	...
	if (data.Display_name) {
		let { name, surname } = converterDisplayNameToNameSurname(data.Display_name)
	}
	if (data.Image) {
		if (imageType === 'URL') {
...
export function converterNameSurnameToDisplayName(name?: string, surname?: string) {
