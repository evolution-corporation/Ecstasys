...
const Base: FC<Props> = props => {
	...
	const animationPress = async () => {
		...
		_opacityButton.value = withSequence(withTiming(0.8, { duration: 500 }), withTiming(1));
	};
	...
	if (colors) {
		...
		WrapperComponent = ({ children }) => (
			<Animated.View style={[animationStyle, button]}>
				<LinearGradient colors={strColors} style={[styles.backgroundButton, styleButton]}>
					{children}
				</LinearGradient>
			</Animated.View>
		);
		...
		WrapperComponent = ({ children }) => (
			<Animated.View style={[styles.backgroundButton, styleButton, animationStyle, button]}>{children}</Animated.View>
		);
	}
