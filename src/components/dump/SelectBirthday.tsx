...
import React, { useReducer, useMemo, useEffect, useState, useRef, useCallback } from "react";
...
const yearPayloadList = createArrayLength(toDay.getFullYear() - 1899).map((_, index) => 1900 + index);
...
const SelectBirthday: FC<SelectBirthdayProps> = props => {
	...
	const [dateList, setDateList] = useState<number[]>(
		createArrayLength(new Date(currentDate.year, currentDate.month + 1, 0).getDate()).map((_, index) => index + 1)
	);
	...
	useEffect(() => {
		...
		setSelectedIsCurrentDate({
			year: toDay.getFullYear() === currentDate.year,
			month: toDay.getFullYear() === currentDate.year && toDay.getMonth() === currentDate.month,
		});
		const _currentDate = new Date(currentDate.year, currentDate.month, currentDate.date);
		...
		<View style={styles.background} {...props}>
			...
			>
				<ScrollPicker
					...
					dataSource={dateList.filter((_, index) => !selectedIsCurrentDate.month || index < toDay.getDate())}
					...
					renderItem={(data, index, isSelected) => {
						return (
							<Text style={[styles.standardTextStyle, { opacity: isSelected ? 1 : 0.5, color: "#FFFFFF" }]}>
								{data}
							</Text>
						);
					}}
				...
				<ScrollPicker
					key={"month"}
					style={styles.dateScroll}
					dataSource={monthPayloadList.filter((_, index) => !selectedIsCurrentDate.year || index <= toDay.getMonth())}
			...
			<Text style={[styles.standardTextStyle, { color: "#FFFFFF", fontSize: 15 }]}>
