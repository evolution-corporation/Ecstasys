...
import { ViewStatisticsMeditation, ColorWithIconButton } from "~components/dump";
import Tools from "~core";
import { AntDesign } from "@expo/vector-icons";
import { useUserContext } from "~modules/account";
import type { ViewStatisticsMeditationType } from "~components/dump/StatisticsMeditation";
...
const Profile: ProfileCompositeScreenProps = ({ navigation }) => {
	...
	const [typeStatisticsMeditation, setTypeStatisticsMeditation] = useState<ViewStatisticsMeditationType>("month");
	...
	const editTimePeriod = useCallback(async (timePeriod: ViewStatisticsMeditationType) => {
		...
		switch (timePeriod) {
			case "week":
				...
				if (coordinate.current.week && coordinate.current.month)
					_positionArrow.value = coordinate.current.week - coordinate.current.month + 7;
				break;
			case "month":
			...
			case "all":
				...
				if (coordinate.current.all && coordinate.current.month)
					_positionArrow.value = coordinate.current.all - coordinate.current.month + 3;
				break;
		}
	...
	return (
		<DoubleColorView style={{ flex: 1, paddingHorizontal: 20 }} heightViewPart={300}>
			<UserInformation />
			...
			<ViewStatisticsMeditation type={typeStatisticsMeditation} colorR={"#7C3D91"} colorL={"#C2A9CE"} />
			<ColorWithIconButton
				icon={<AntDesign name="heart" size={24} color="#F83047" style={{ marginLeft: 15 }} />}
			...
			<ColorWithIconButton
				icon={<AntDesign name="star" size={24} color="#FBBC05" style={{ marginLeft: 15 }} />}
