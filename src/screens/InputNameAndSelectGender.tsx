...
const InputNameAndSelectGender:RootScreenProps<"InputNameAndSelectGender"> = ({ navigation }) => {
    ...
    return (
        <View style={{ flex: 1 }}>
            ...
            <View style={{ width: '100%', backgroundColor: '#FFF', borderTopLeftRadius: 20, borderTopRightRadius: 20, paddingHorizontal: 20, paddingVertical: 27, alignItems: 'center', position: 'absolute', bottom: 0 }}>
                <Text style={{ fontSize: 20, ...gStyle.font('700'), textAlign: 'center', color: "#3D3D3D" }}>{i18n.t('9c680da5-0b16-4861-8d16-a0a121dc3ed1')}</Text>
                ...
                />
                 ...
                 <View style={{ flexDirection: 'row', height: 163, width: '100%', marginTop: 13, marginBottom: 45 }}>
                    <Pressable style={{ flex: 1, alignItems: 'center' }} onPress={()=>{
                        ...
                        <Image source={require('assets/man.png')} style={{ flex: 1, width: '100%' }} resizeMode={'center'}/>
                        <View style={[styles.generalButton, gender === Gender.MALE ? {  backgroundColor: "#9765A8"} : null]}>
                            <Text style={[styles.generalText, gender === Gender.MALE ? {  color: "#FFF"} : null]}>{i18n.t('M')}</Text>
                        </View>
                    </Pressable>
                    <View style={{ width: 38, height: '100%', justifyContent: 'flex-end' }}>
                        <Pressable style={[styles.generalButton, gender === Gender.OTHER ? {  backgroundColor: "#9765A8"} : null]} onPress={()=>{
                            setGender(Gender.OTHER)
                        }}>
                            <View style={[{ backgroundColor: "#9765A8", width: 11, height: 1 }, gender === Gender.OTHER ? {  backgroundColor: "#FFF"} : null]} />
                        </Pressable>
                    </View>
                    <Pressable style={{ flex: 1, alignItems: 'center' }} onPress={()=>{
                        setGender(Gender.FEMALE)
                    }}>
                        <Image source={require('assets/fam.png')} style={{ flex: 1, width: '100%' }} resizeMode={'center'}/>
                        <View style={[styles.generalButton, gender === Gender.FEMALE ? {  backgroundColor: "#9765A8"} : null]}>
                            <Text style={[styles.generalText, gender === Gender.FEMALE ? {  color: "#FFF"} : null]}>{i18n.t('W')}</Text>
                        </View>
                    </Pressable>
                    
                </View></>}
                <ColorButton styleText={{ color: "#FFF" }} styleButton={{ backgroundColor: "#C2A9CE", paddingHorizontal: 25, marginBottom: 20 }} onPress={()=>{
                    save()
                }}>{i18n.t('save')}</ColorButton>
                <TextButton styleText={{ color: "#9765A8"}} onPress={()=> {dispatch(actions.setNotNewUser()); navigation.goBack()}}>{i18n.t('9dca1ae3-981c-4c23-a38e-62d5eb7a23db')}</TextButton>
            </View>
        </View>
    )
