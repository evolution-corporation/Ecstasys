...
import { PressableProps, StyleSheet, TouchableOpacity, View, ColorValue, ViewProps, ImageStyle } from "react-native";
...
export const ArrowButtonMask: FC<ArrowButtonProps> = props => {
	...
	return (
		<TouchableOpacity style={[styles.background, { backgroundColor }]} onPress={!!onPress ? onPress : () => {}}>
			<MaskedView
...
export const ArrowButton: FC<PressableProps & { color?: ColorValue }> = props => {
	const { style, onPress = () => {}, color = "black" } = props;
	if (typeof style === "function") throw new Error("Style not Function");
	return (
		<TouchableOpacity
			style={[styles.background, { alignItems: "center", justifyContent: "center" }, style]}
