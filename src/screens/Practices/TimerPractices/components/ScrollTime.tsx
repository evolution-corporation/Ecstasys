...
import React, { ElementRef, FC, useEffect, useMemo, useRef, useState } from "react";
	...
	useEffect(() => {
		if (onChange)
			onChange({
				...
				seconds: seconds.filter(item => (selectedMinutesIndex === 0 ? item >= minimalSeconds : true))[
		...
		<View style={[styles.selectedTime, style]}>
			<View style={styles.wrapperScroll} key={"minuts"}>
				<ScrollPicker
					...
					renderItem={(data, index, isSelected) => {
						return (
							<Text style={[styles.textItem, { color: isSelected ? "#9765A8" : "#D3D3D3" }]}>
			...
			<View style={styles.wrapperScroll} key={"seconds"}>
				<ScrollPicker
					key={"seconds"}
					style={styles.timeScroll}
					dataSource={seconds.filter(item => (selectedMinutesIndex === 0 ? item >= minimalSeconds : true))}
